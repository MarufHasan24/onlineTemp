...
module.exports = {
  ...
  submit: function (req, res) {
    let { body, devider, username } = req.body;
    devider = devider || ";";
      ...
      (err, oldata, callback) => {
        if (err) {
          ...
          let wordcount = oldata.data.word_count,
            precondition =
              Number(oldata.data.pre_condition) > 0
                ? Number(oldata.data.pre_condition)
                : 0,
                    ...
                    getinfo(
                      ...
                      (err, data) => {
                        if (err.length) {
                          ...
                          const beautifiedJSON = removeCircularReferences(err);
                          state[e] = {
                            result: "error",
                            time: Date.now(),
                            cause:
                              typeof err === "object" ? beautifiedJSON : err,
                          };
                        } else {
                          if (
                            ...
                            state[e] = {
                              result: "info",
                              time: Date.now(),
                              cause: `You has created <b>${e}</b> in ${new Date(
                                data.cdata.creation
  ...
  getRows: function (req, res) {
    let key = req.body?.key;
    let count = req.body?.count || 500;
    let format = req.body?.format || "html";
    let startindex = parseInt(req.body?.startindex, 10) || 0;
    ...
    readFile(
      ...
      (err, rdata) => {
        if (err) {
          ...
          let tempagelist = {};
          let table = {};
          let endindex = Math.min(startindex + count, keys.length);
                            ...
                            if (err) {
                              ...
                              callback(rdata, (err) => {
                                if (err) {
                                  ...
                                  return res.status(200).send({
                                    message:
                                      "Result added successfully. return body: <br>" +
                                      JSON.stringify(edata, null, 2),
                                    type: "success",
                                    redirect: null,
                                  });
