...
function updateFileOwn(username, data, callback) {
  ...
  fs.stat(filePath, (err, stats) => {
    if (err) {
      ...
      keepLog(username, stats, "hosting", (err) => {
        if (err) {
          console.error(err);
        } else {
...
function deleteFile(path, callback, key, username) {
  fs.stat(path, (err, stats) => {
    if (err) {
      ...
      keepLog(username, { ...stats, key: key || null }, "delete", (err) => {
        if (err) {
          console.error(err);
        } else {
...
function keepLog(username, rdata, action, callback) {
  let sdata = {};
  sdata.atime = rdata.atime || null;
  sdata.size = rdata.size || null;
  sdata.mtime = rdata.mtime || null;
  sdata.ctime = rdata.ctime || null;
  sdata.birthtime = rdata.birthtime || null;
  if (username) {
    const date = new Date();
